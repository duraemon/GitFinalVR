...
public class HandComplete : MonoBehaviour
{
   ...
   // public TimeControlVR2 timeCo;
    ...
    private void Start()
    {
        ...
        InitializeHand();
        if (GetComponent<TimeControlVR2>() != null)
        {
            Debug.Log("Checking");
    ...
    private void UpdateHand()
    {
        ...
        //if (_targetDevice.TryGetFeatureValue(CommonUsages.primaryButton, out float gripValue))

            //This will get the value for our grip from the target device and output a flaot into gripValue
            if (_targetDevice.TryGetFeatureValue (CommonUsages.trigger, out float triggerValue))
        {
            _handAnimator.SetFloat("Grip", triggerValue);
        }
        else
        {
